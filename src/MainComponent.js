    ...
    try {
      ...
      let data = await res.json();
        if(data && data.main && data.wind){
            ...
            let finalSunrise = `${new Date(sunrise1).getHours()} : ${new Date(
              sunrise1
            ).getMinutes()}`;
            let sunset1 = new Date(new Date(sunset * 1000)).toUTCString();
            let finalSunset = `${new Date(sunset1).getHours()} : ${new Date(
              sunset1
            ).getMinutes()}`;
      ...
      <div className="main-div">
        <div className="input-div">
          ...
          <div className="allDetails-div">
            <div className="main-icon-div">{mainIcon}</div>
            <div className="temp-date-div">
              <div className="temp-div">
                ...
                <p>
                  {finalData.weathermood} <br />
                  <span>
                    {finalData.name}, {finalData.country}
                  </span>
                </p>
              </div>
            ...
            <div className="last-div">
              <HumidityDiv
                icon={<WiSunrise />}
                value={finalData.finalSunrise}
                text={"Sunrise"}
              />
              <HumidityDiv
                icon={<WiSunset />}
                value={finalData.finalSunset}
                text={"Sunset"}
              />
              <HumidityDiv
                icon={<WiHumidity />}
                value={finalData.humidity}
                text={"Humidity"}
              />
              <HumidityDiv
                icon={<WiStrongWind />}
